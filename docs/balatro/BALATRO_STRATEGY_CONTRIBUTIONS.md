...
Canonical component/state -> Bond contribution values after **Catalogue Audit Pass 3** plus the No Face Cards mechanical correction. Architecture is in `BALATRO_STRATEGY_SYSTEM.md`; identities/unlocks are in `BALATRO_STRATEGY_CATALOGUE.md`.
...
**No Face Cards** `4/8/10/12/14` — hard unlock Ride the Bus. Ride the Bus +7. Observed natural J/Q/K count `<=9 / <=6 / <=3 / 0` contributes `+1 / +3 / +5 / +7`. Empty/unknown deck state contributes no density quota. Generic face-card interaction contributes `0`.
...
- No Face Cards was admitted after freeze because Ride the Bus creates a genuine independent inverse-payoff/deck-shaping axis; it is a mechanical omission correction rather than speculative expansion.
