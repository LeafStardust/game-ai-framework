...
- Audited the 2026-08-25 Ante-4 Mouth loss and corrected five independent decision defects: once The Mouth locks a hand type, zero-score off-type plays are filtered while recovery exists and equal-structure discards maximize redraw width; isolated conditional hand Jokers such as Wily no longer open unsupported axes after the first Joker; Magic Trick/Illusion are treated as shop dilution without a Hologram card-add payoff; Celestial pack odds now use the finite twelve-Planet catalogue instead of logging a one-target Jumbo pack as a near-certain hit; and the permanent deck now satisfies `target:card` so a held Death cannot fabricate Standard-pack demand.
