...
Unsupported discard callbacks remain fail-closed until their canonical effects
are wired here rather than approximated by the tactical planner.
"""
...
def _require_private_public_round_zones(run: HeadlessRunState) -> None:
    ...
    if run.played_pile:
        raise HeadlessTransitionError(
            "tactical discard requires an empty played pile"
        )
...
def apply_supported_tactical_discard(
    ...
    if state.phase != "SELECTING_HAND":
        raise HeadlessTransitionError(
            "tactical discard requires SELECTING_HAND phase"
        )
    ...
    # Validate the full permanent-card partition before copying/mutating.  This
    # also prevents a public-only hand from being accepted when hidden physical
    # state is stale.
    ...
    # physical deck tail.  Each primitive draw also restores vanilla hand sort
    # and canonicalizes the public deck without exposing hidden draw order.
