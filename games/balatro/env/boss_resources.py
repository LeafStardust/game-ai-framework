"""Exact reversible resource mutations for Water/Needle Boss blinds.
...
_RESOURCE_BOSS_NAMES = frozenset({"The Water", "The Needle"})


def apply_resource_boss_start(run: HeadlessRunState) -> HeadlessRunState:
    """Apply exact Water/Needle ``Blind:set_blind`` resource mutation."""
    state = run.public
    ...
    if run.boss_hands_sub is not None or run.boss_discards_sub is not None:
    ...
    if isinstance(reset_hands, bool) or not isinstance(reset_hands, int) or reset_hands < 0:
        raise HeadlessTransitionError(
...
def disable_resource_boss(run: HeadlessRunState) -> HeadlessRunState:
    """Mirror Water/Needle ``Blind:disable`` restoration and clear private state."""
    state = run.public
    next_run = run.copy()
    next_state = next_run.public
