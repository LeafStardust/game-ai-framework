...
_START_INERT_BOSS_NAMES = frozenset({"The Psychic", "The Flint", "The Tooth"})
...
def prepare_supported_start_inert_boss_start(run: HeadlessRunState) -> HeadlessRunState:
    ...
    The Psychic, The Flint, and The Tooth do not mutate state in vanilla
    ``Blind:set_blind``. Their actual effects occur later during hand legality,
    scoring, or played-card economy processing respectively. Admitting them here
    therefore means only that their blind-start lifecycle is exact; it does not
    claim those downstream mechanics are headless-owned.
    """
    _require_boss_blind(run, label="start-inert boss start")
    if run.public.boss_name not in _START_INERT_BOSS_NAMES:
        raise HeadlessTransitionError(
            "boss is not in the audited start-inert start set"
        )
    return _apply_common_predeal_lifecycle(run)


def start_supported_start_inert_boss(run: HeadlessRunState) -> HeadlessRunState:
    """Compose an audited start-inert Boss boundary with exact shuffle/deal."""
    prepared = prepare_supported_start_inert_boss_start(run)
    return deal_supported_round_start(prepared)
