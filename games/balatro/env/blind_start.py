...
def prepare_supported_start_inert_boss_start(run: HeadlessRunState) -> HeadlessRunState:
    ...
    The Psychic, The Flint, The Tooth, The Hook, The Ox, The Arm, and The
    Serpent do not mutate state in vanilla ``Blind:set_blind`` or initial
    ``Blind:drawn_to_hand``. Their actual effects occur later during hand
    scoring, ``Blind:press_play`` processing, or post-action draw. Admitting
    them here therefore means only that their blind-start lifecycle is exact;
    downstream effects remain separately owned/tested.
    """
