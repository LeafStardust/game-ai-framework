...
because pack contents are already sunk after opening. The result is a conservative
lower bound for packs with multiple visible offers: choosing the best of two/four
Jokers cannot be worse than the expectation of one offer. We deliberately do not
multiply by an independence assumption or inspect hidden pack contents.
"""
    ...
    def recommend(self, state, action):
        ...
        if price > int(state.money):
            return ShopBoosterRecommendation(
                ...
                rationale=(
                    f"Buffoon pack costs ${price} but only ${state.money} is available",
                ),
            )
