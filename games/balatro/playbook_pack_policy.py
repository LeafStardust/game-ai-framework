...
class PlaybookBalatroPackPolicy(BalatroPackPolicy):
    """D9/D10 deck/stake threshold adapter over canonical pack mechanics."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._skip_bias_override = None if skip_bias is None else float(skip_bias)
    ...
    def _buffoon_replacement_score(self, state, action, choice):
        ...
        if not isinstance(data, dict):
            return PackActionScore(action, -1.0, ("full-roster Buffoon Joker cannot be modeled for replacement",))
        ...
        if candidate is None or evaluator is None:
            return PackActionScore(action, -1.0, ("full-roster Buffoon Joker replacement evaluator unavailable",))

        policy = PlaybookJokerAcquisitionPolicy(JokerBuildTransitionPlanner(evaluator=evaluator))
        ...
        if decision.action != REPLACE or selected is None or selected.replace_index is None:
            return PackActionScore(
                action,
                -1.0,
                ("visible Buffoon Joker does not justify replacing an incumbent", *decision.rationale),
            )
    ...
    def score_action(self, state, action):
        if action.name == SKIP_BOOSTER:
            ...
            return PackActionScore(action, bias, (f"skip booster; D9 skip_bias={bias:.3f}",))
        ...
        if getattr(choice, "kind", None) == "TAROT" and getattr(choice, "label", None) == "Judgement":
            joker_slots = max(0, int(getattr(state, "joker_slots", 5) or 5))
            free_slots = max(0, joker_slots - len(getattr(state, "jokers", ()) or ()))
            if free_slots > 0:
                value = self.JUDGEMENT_EMPTY_SLOT_BASE_VALUE + min(3, free_slots) * self.JUDGEMENT_EMPTY_SLOT_BONUS
                return PackActionScore(
                    action,
                    value,
                    (
