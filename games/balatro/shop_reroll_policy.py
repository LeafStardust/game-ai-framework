...
VANILLA_SHOP_REROLL_PRIOR = ShopRerollPoolPrior(
    card_slots=2,
    offers=(
        FutureShopOfferPrior("JOKER", 20.0, 6.0, 5, "JOKER"),
        FutureShopOfferPrior("TAROT", 4.0, 3.2, 3, "CONSUMABLE"),
        FutureShopOfferPrior("PLANET", 4.0, 3.5, 3, "CONSUMABLE"),
    ),
)
...
class BuildAwareShopRerollPolicy:
    ...
    def recommend(
        ...
        *,
        ...
        current_best = current_scores[0].total if current_scores else float(self.shop_policy.hold_bias)
        ...
        if reroll_cost is None:
            return self._fail_closed(current_best=current_best, reroll_cost=None, unmet=unmet, reason="current reroll cost is not observed; reroll fails closed")

        cost = int(reroll_cost)
        if cost < 0:
            raise ValueError("reroll cost cannot be negative")
        if cost > state.money:
            return self._fail_closed(current_best=current_best, reroll_cost=cost, unmet=unmet, reason=f"reroll costs ${cost} but only ${state.money} is available")

        prior = self.pool_prior
        if prior is None or not prior.is_valid():
            return self._fail_closed(current_best=current_best, reroll_cost=cost, unmet=unmet, reason="public/static future-shop pool prior is unavailable; reroll fails closed")
        ...
        future_ev, offer_scores = self._future_shop_ev(state, prior, money_after_reroll=money_after_reroll)
        reroll_score = future_ev - reroll_resource.total
        required = current_best + (0.0 if cost == 0 else self.thresholds.minimum_margin)

        rationale = (
            f"visible-shop best score={current_best:.3f}",
            f"future shop EV={future_ev:.3f} across {prior.card_slots} card slots",
            "public pool weights=" + ", ".join(f"{offer.family}:{offer.weight:g}" for offer in prior.offers),
            "actionable offer scores=" + ", ".join(f"{family}:{score:.3f}" for family, score in offer_scores),
        ...
        if hold:
            return ShopRerollRecommendation("HOLD", cost, None, current_best, future_ev, reroll_resource.total, reroll_score, unmet, rationale)
        return ShopRerollRecommendation("REROLL", cost, BalatroAction(REFRESH_SHOP), current_best, future_ev, reroll_resource.total, reroll_score, unmet, rationale)

    def _fail_closed(self, *, current_best: float, reroll_cost: int | None, unmet: tuple[str, ...], reason: str) -> ShopRerollRecommendation:
        return ShopRerollRecommendation("HOLD", reroll_cost, None, current_best, float("-inf"), float("inf"), float("-inf"), unmet, (reason, "no heuristic exploration fallback is used"))

    def _future_shop_ev(self, state: BalatroState, prior: ShopRerollPoolPrior, *, money_after_reroll: int) -> tuple[float, tuple[tuple[str, float], ...]]:
        ...
        scores = tuple(self._future_offer_score(state, offer, money=money_after_reroll) for offer in prior.offers)
    ...
    def _future_offer_score(self, state: BalatroState, offer: FutureShopOfferPrior, *, money: int) -> float:
        hold = float(self.shop_policy.hold_bias)
        ...
        if offer.resource == "JOKER":
            ...
            slot_cost = self.shop_policy.resource_valuator.slot_opportunity_cost(
                occupied=len(state.jokers), capacity=state.joker_slots,
            ...
            if len(state.consumables) >= state.consumable_slots:
                return hold
            slot_cost = self.shop_policy.resource_valuator.slot_opportunity_cost(
                occupied=len(state.consumables), capacity=state.consumable_slots,
                last_slot_penalty=self.shop_policy.last_consumable_slot_penalty,
                penultimate_slot_penalty=0.0, resource="consumable",
            ).total
        else:
            return hold
    ...
    def _visible_scores(self, state: BalatroState, visible_actions: list[BalatroAction]) -> list[ShopActionScore]:
    ...
    def _unmet_requirements(self, state: BalatroState) -> tuple[str, ...]:
        ...
        requirements = {requirement for effect in profile.effects for requirement in effect.requires}
        return tuple(sorted(requirement for requirement in requirements if not profile.supports(requirement)))
