...
def install_hook_planner_integration_policy() -> None:
    """Keep The Hook's stochastic score branches authoritative in D1 search.

    The final score-outcome layer already branches exactly over The Hook's two
    random forced discards and applies the normal discard-Joker event semantics.
    The generic blind planner previously rebuilt every child from a common retained
    hand, which reintroduced cards that a Hook branch had already discarded.

    For Hook only, derive the retained hand and replacement draw count from each
    score outcome's own state. No second Hook event is generated here.
    """
