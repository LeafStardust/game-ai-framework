...
def install_serpent_draw_policy() -> None:
    """Keep exact Serpent draw semantics on the reusable base planner.

    Integrated production D1 also models the rule natively. The base planner remains
    a public mechanics/projection surface used by deterministic tests and utilities,
    so it must preserve the same exact transition semantics.
    """
