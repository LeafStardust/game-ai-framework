...
def _snapshot_from_log_state(value: Any) -> LiveBalatroSnapshot:
    ...
    return LiveBalatroSnapshot(sequence=sequence, phase=phase, state_complete=state_complete, payload=payload)
...
def successful_tactical_evidence_from_run_rows(rows: Iterable[dict[str, Any]], *, translator: DefaultBalatroStateTranslator | None = None) -> tuple[PublicTacticalTransitionEvidence, ...]:
    ...
    for row in rows:
        ...
        evidence.append(PublicTacticalTransitionEvidence(before=before, action=balatro_action, selected_hand_indices=indices, after=after))
        pending_decision = None
    return tuple(evidence)


def compare_run_rows_to_simulator_tactical_evidence(rows: Iterable[dict[str, Any]], simulator_evidence: Iterable[PublicTacticalTransitionEvidence], *, translator: DefaultBalatroStateTranslator | None = None) -> PublicTacticalTrajectoryParityComparison:
    live_evidence = successful_tactical_evidence_from_run_rows(rows, translator=translator)
    return compare_public_tactical_trajectory(live_evidence, simulator_evidence)


def successful_reroll_shop_evidence_from_run_rows(rows: Iterable[dict[str, Any]], *, translator: DefaultBalatroStateTranslator | None = None) -> tuple[PublicStrategicTransitionEvidence, ...]:
    ...
    for row in rows:
        ...
        if event == "decision":
            ...
            if str(action.get("name") or "") == REFRESH_SHOP:
                ...
                if last_observation is None:
                    raise ValueError("REFRESH_SHOP decision has no preceding observation")
        ...
        if event != "action_result":
            continue
        action = data.get("action")
        if not isinstance(action, dict) or str(action.get("name") or "") != REFRESH_SHOP:
            continue
        if action != _REROLL_SHOP_LOG_ACTION:
            raise ValueError("REFRESH_SHOP run-log action must not contain parameters")
        if last_observation is None or pending_reroll is None:
            raise ValueError("REFRESH_SHOP action_result has no captured decision boundary")
        if action != pending_reroll:
            raise ValueError("REFRESH_SHOP action_result does not match captured decision")
        ...
        evidence.append(build_public_strategic_transition_evidence(before, EnvAction.from_alias("REROLL_SHOP"), after))
        pending_reroll = None
