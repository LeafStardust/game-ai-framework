...
def project_destroyed_playing_cards(state, cards) -> tuple:
    """Apply permanent public-state consequences of deterministic destruction.

    The caller owns transient hand/deck-area movement. This helper updates only
    permanent playing-card composition and Joker state that reacts to a destroyed
    playing card, so it can be reused by discard, played-hand and consumable
    projections without shifting live hand indices prematurely.
    """
    if state is None:
        return ()
