...
from .blind_mouse import BLIND_TARGETS, BlindMouseLayout, ExternalBlindMouseExecutor
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Preview or execute exactly one calibrated Balatro blind Select click."
        )
    )
    parser.add_argument("target", choices=sorted(BLIND_TARGETS))
    ...
    parser.add_argument(
        "--execute",
        action="store_true",
        help="arm mouse input and click exactly this one blind Select button",
    )
    ...
    try:
        ...
        point = layout.point_for(args.target)
    ...
    print(f"Target -> {args.target.upper()} Blind")
    print(f"Point -> click({point.x:.4f},{point.y:.4f})")

    if not args.execute:
        print("Mouse input sent -> False")
        print("Re-run with --execute to send exactly this one blind selection click.")
        return 0

    mouse = BalatroMouseController(armed=True)
    try:
        with ExternalBlindMouseExecutor(layout, mouse=mouse) as executor:
            executor.dispatch(args.target)

        print("Mouse input sent -> True")
        print("Waiting for save checkpoint -> SELECTING_HAND")
