...
EXPECTED_BUTTON = "cash_out"
EXPECTED_ID = "cash_out_button"
REQUIRED_GEOMETRY = ("x", "y", "w", "h")


class LiveRoundEvalMouseError(RuntimeError):
    pass


@dataclass(frozen=True)
class LiveRoundEvalTarget:
    node_address: int
    button: str
    control_id: str
    geometry_source: str
...
def resolve_live_cash_out_target(
    decoder,
    root: dict,
    client_rect: WindowRect,
) -> LiveRoundEvalTarget:
    """Resolve the visible ROUND_EVAL Cash Out control from live UI memory.

    The resolver follows only the public UI/controller path that Balatro itself
    uses for cursor snapping. It does not traverse deck order or RNG state.
    """

    controller = _table_fields(decoder, root.get("CONTROLLER"))
    if not controller:
        raise LiveRoundEvalMouseError("G.CONTROLLER is unavailable or not a table")

    snap_cursor_to = _table_fields(decoder, controller.get("snap_cursor_to"))
    if not snap_cursor_to:
        raise LiveRoundEvalMouseError(
    ...
    if not node:
        raise LiveRoundEvalMouseError("unable to read cash-out owner node")
    ...
    if button != EXPECTED_BUTTON or control_id != EXPECTED_ID:
        raise LiveRoundEvalMouseError(
            "snap cursor node is not the Cash Out control: "
...
class LiveMemoryRoundEvalMouseExecutor:
    """Click Cash Out using live UI geometry and normal desktop mouse input."""
    ...
    def preview(
        self,
    ) -> tuple[LiveBalatroSnapshot, LiveRoundEvalTarget, BalatroWindow]:
        window = self.window_locator.find()
        snapshot = self.observer.observe()
        if snapshot.phase != "ROUND_EVAL":
            raise LiveRoundEvalMouseError(
                f"Balatro is in {snapshot.phase}, expected ROUND_EVAL"
            )
    ...
    def dispatch(
        self,
    ) -> tuple[LiveBalatroSnapshot, LiveRoundEvalTarget]:
        window = self.window_locator.find()
    ...
    def _wait_for_foreground(self, handle: int) -> None:
        ...
        while True:
            ...
            if time.monotonic() >= deadline:
                raise LiveRoundEvalMouseError(
                    "Balatro did not become foreground before Cash Out click"
                )
    ...
    def __enter__(self) -> "LiveMemoryRoundEvalMouseExecutor":
        return self
