...
def _guard_errors(
    ...
    *,
    ...
    if money is None:
        ...
        errors.append(f"expected money {expected_money:g}, observed {money:g}")
    ...
    if item is None:
        ...
        errors.append(f"{area_name} index {index} is not currently available")
        return errors
    ...
    if expected_cost is None:
        ...
        errors.append(f"expected cost {expected_cost:g}, observed {observed_cost:g}")

    ability_set = _ability_set(item)
    if action_name == BUY_JOKER and ability_set != "JOKER":
        errors.append(f"BUY_JOKER target is {ability_set or '<unknown>'}, not JOKER")
...
def main() -> int:
    ...
    with LiveMemoryBalatroObserver() as observer:
        ...
        print("Supported injected shop actions -> " + ", ".join(_ACTIONS))
        ...
        if latest.sequence != snapshot.sequence or _fingerprint(latest) != _fingerprint(snapshot):
            print("Execution guard -> BLOCKED")
            print(
