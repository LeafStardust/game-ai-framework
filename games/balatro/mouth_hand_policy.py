...
"""Compatibility helper for retired first-hand Mouth refinement.

Production no longer installs a Mouth post-policy selector. Exact Mouth constraints
and locked-hand redraw evidence live in canonical D1. This module intentionally
installs nothing; it preserves the historical pure helper used by semantic and
legacy deterministic tests, with the canonical action class kept immutable.
"""
...
def _hand_type(policy, state, plan) -> str:
    ...
    try:
        result = policy._hand_evaluator.evaluate(cards, rules=hand_rules_for_state(state))
    except TypeError:
        result = policy._hand_evaluator.evaluate(cards)
...
def apply_mouth_first_hand_policy(policy, state, plans, decision):
    """Legacy pure refinement that never changes the canonical action class."""
    ...
    if one_shot:
        ...
        return _replace_with_play(
            ...
            plan,
            rationale=(
                f"The Mouth first-hand lock is irrelevant because {hand_type} immediately clears the blind ({score:.3f} projected)",
            ),
        )
...
__all__ = ["apply_mouth_first_hand_policy"]
