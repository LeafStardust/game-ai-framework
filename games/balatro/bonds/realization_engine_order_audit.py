...
def realize_vampire_ordered(d,s):
 ...
 sc=_cards(s,"scoring_cards","played_cards","current_played_cards");hand=_cards(s,"hand","current_hand","cards_in_hand");deck=_cards(s,"owned_deck","deck");feed_pool=sc or hand or deck;feed=sum(1 for c in feed_pool if not _debuffed(c) and str(getattr(c,"enhancement","") or "").strip());midas=any("midasmask" in _name(x) for x in j);par=any("pareidolia" in _name(x) for x in j);pool=sc or hand or deck;renew=midas and any(_face(c,par) for c in pool);a=feed>0 or renew;return _finish(d,a,feed>=2 or (renew and int(getattr(s,"vampire_enhancements_consumed",0) or 0)>=15))
