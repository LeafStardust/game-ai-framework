...
def realize_card_destruction_scoring(dev: BondDevelopment, state: Any) -> BondDevelopment:
    jokers = _jokers(state)
    names = {_name(joker) for joker in jokers}
    ...
    glass_pool = list(scoring_raw or ()) if scoring_raw is not None else _cards(state, "cards_to_play", "selected_cards", "hand", "current_hand")
    glass = any("glassjoker" in name for name in names) and any(
        str(getattr(card, "enhancement", "") or "").lower() == "glass" for card in glass_pool
    )

    canio = any("canio" in name for name in names) and int(getattr(state, "cards_destroyed", 0) or 0) > 0
...
def realize_vampire_ordered(dev: BondDevelopment, state: Any) -> BondDevelopment:
    jokers = _jokers(state)
    vampire_index = next((i for i, joker in enumerate(jokers) if "vampire" in _name(joker)), None)
    if vampire_index is None:
        return _finish(dev, False)

    scoring_raw = getattr(state, "scoring_cards", None)
    if scoring_raw is not None:
        scoring = list(scoring_raw or ())
        feed_cards = scoring
    else:
        scoring = _cards(state, "played_cards", "current_played_cards")
        feed_cards = scoring or _cards(state, "hand", "current_hand", "cards_in_hand")

    feed = sum(1 for card in feed_cards if str(getattr(card, "enhancement", "") or "").strip())
    midas_index = next((i for i, joker in enumerate(jokers) if "midasmask" in _name(joker)), None)
    ...
    active = feed > 0 or same_hand_midas_feed
    strong = feed >= 2 or (
        same_hand_midas_feed and int(getattr(state, "vampire_enhancements_consumed", 0) or 0) >= 15
    )
    return _finish(dev, active, strong)


ENGINE_AUDIT_REALIZERS = {
    "joker_sacrifice": realize_joker_sacrifice_ordered,
    "card_destruction": realize_card_destruction_scoring,
    "vampire": realize_vampire_ordered,
}
