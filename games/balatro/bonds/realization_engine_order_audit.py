...
def realize_card_destruction_scoring(d,s):
 j=_jokers(s);names={_name(x) for x in j};hand=_cards(s,"hand","current_hand","cards_in_hand");fd=bool(getattr(s,"first_discard_available",int(getattr(s,"discards_used_this_round",0) or 0)==0));fh=bool(getattr(s,"first_hand_available",int(getattr(s,"hands_played_this_round",0) or 0)==0));has_discard_window=any(hasattr(s,n) for n in ("cards_to_discard","selected_cards"));has_scoring=hasattr(s,"scoring_cards");has_play_window=has_scoring or any(hasattr(s,n) for n in ("cards_to_play","played_cards","current_played_cards"));sd=_cards(s,"cards_to_discard","selected_cards");sp=_cards(s,"cards_to_play","selected_cards");trading=any("tradingcard" in n for n in names) and fd and ((len(sd)==1) if has_discard_window else bool(hand));six=any("sixthsense" in n for n in names) and fh and ((len(sp)==1 and not _debuffed(sp[0]) and not _stone(sp[0]) and str(getattr(sp[0],"rank","") or "")=="6") if has_play_window and sp else any(not _debuffed(c) and not _stone(c) and str(getattr(c,"rank","") or "")=="6" for c in hand));pool=_cards(s,"scoring_cards") if has_scoring else (_cards(s,"cards_to_play","played_cards","current_played_cards") or hand);glass=any("glassjoker" in n for n in names) and any(not _debuffed(c) and str(getattr(c,"enhancement","") or "").lower()=="glass" for c in pool);canio=_canio_live(s,j);a=trading or six or glass or canio;return _finish(d,a,sum((trading,six,glass,canio))>=2)
