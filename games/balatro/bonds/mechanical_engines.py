...
def _locked(bond_id: str, thresholds: dict[BondRank, float]) -> BondDevelopment:
    return BondDevelopment(
        bond_id=bond_id,
        unlocked=False,
        contribution=0.0,
        rank=BondRank.LOCKED,
        next_rank_threshold=thresholds[BondRank.R1],
        contributions=(),
    )
        ...
        for mechanic, value, fallback in specs:
            if component_has_mechanic(component, mechanic):
                parts.append(component_contribution(
                    ...
                    index=index,
                    label=_label(component, fallback),
                    value=value,
                    mechanic=mechanic,
                ))
                break
...
def evaluate_discard_bond(state: Any) -> BondDevelopment:
    ...
    if not any(components_have_mechanic(jokers, mechanic) for mechanic in defining):
        return _locked("discard", DISCARD_THRESHOLDS)
    ...
    if extra:
        parts.append(state_contribution(
            "round:extra_discard_capacity",
            "Extra discard capacity",
            float(min(4, extra)),
            mechanic="extra_discard_capacity",
        ))
    return finalize_development("discard", parts, DISCARD_THRESHOLDS)


def evaluate_blind_skip_bond(state: Any) -> BondDevelopment:
    jokers = list(getattr(state, "jokers", ()) or ())
    if not components_have_mechanic(jokers, BLIND_SKIP_SCALING):
        return _locked("blind_skip", BLIND_SKIP_THRESHOLDS)
    parts = _mechanic_parts(jokers, (
    ...
    if score:
        parts.append(state_contribution(
            "history:blinds_skipped", "Blind-skip history", score,
            mechanic="blind_skip_history",
        ))
    return finalize_development("blind_skip", parts, BLIND_SKIP_THRESHOLDS)


def evaluate_sell_value_bond(state: Any) -> BondDevelopment:
    jokers = list(getattr(state, "jokers", ()) or ())
    if not components_have_mechanic(jokers, SELL_VALUE_SCORING):
        return _locked("sell_value", SELL_VALUE_THRESHOLDS)
    parts = _mechanic_parts(jokers, (
    ...
    if score:
        parts.append(state_contribution(
            "jokers:sell_value_total", "Current Joker sell value", score,
            mechanic="joker_sell_value_total",
        ))
    return finalize_development("sell_value", parts, SELL_VALUE_THRESHOLDS)
...
def evaluate_joker_sacrifice_bond(state: Any) -> BondDevelopment:
    ...
    if not (
        ...
        return _locked("joker_sacrifice", JOKER_SACRIFICE_THRESHOLDS)
    parts = _mechanic_parts(jokers, (
    ...
    if score:
        parts.append(state_contribution(
            "history:jokers_destroyed", "Destroyed Joker history", score,
            mechanic="joker_destruction_history",
        ))
    return finalize_development("joker_sacrifice", parts, JOKER_SACRIFICE_THRESHOLDS)
...
def evaluate_card_destruction_bond(state: Any) -> BondDevelopment:
    ...
    if not defining:
        return _locked("card_destruction", CARD_DESTRUCTION_THRESHOLDS)
    parts: list[BondContribution] = []
    for index, component in enumerate(jokers):
        if component_has_mechanic(component, FACE_DESTRUCTION_SCALING):
            mechanic, value, fallback = FACE_DESTRUCTION_SCALING, 7.0, "Face-destruction scaler"
        elif component_has_mechanic(component, GLASS_DESTRUCTION_SCALING):
            mechanic, value, fallback = GLASS_DESTRUCTION_SCALING, 3.0, "Glass-destruction scaler"
            ...
            fallback = "Card-destruction engine"
        else:
            continue
        parts.append(component_contribution(
            component, collection="jokers", index=index,
            label=_label(component, fallback), value=value, mechanic=mechanic,
        ))
    destroyed = int(getattr(state, "cards_destroyed", 0) or 0)
    score = _band(destroyed, ((2, 1.0), (5, 3.0), (10, 5.0), (16, 7.0)))
    if score:
        parts.append(state_contribution(
            "history:cards_destroyed", "Destroyed playing-card history", score,
            mechanic="card_destruction_history",
        ))
    return finalize_development("card_destruction", parts, CARD_DESTRUCTION_THRESHOLDS)
...
def evaluate_hand_repetition_bond(state: Any) -> BondDevelopment:
    ...
    if not (
        ...
        return _locked("hand_repetition", HAND_REPETITION_THRESHOLDS)
    parts = _mechanic_parts(jokers, (
    ...
    if score:
        parts.append(state_contribution(
            "history:max_hand_repetition", "Repeated hand history", score,
            mechanic="hand_repetition_history",
        ))
    return finalize_development("hand_repetition", parts, HAND_REPETITION_THRESHOLDS)
...
def evaluate_enhanced_cards_bond(state: Any) -> BondDevelopment:
    ...
    if not components_have_mechanic(jokers, ENHANCEMENT_DENSITY_PAYOFF):
        return _locked("enhanced_cards", ENHANCED_CARDS_THRESHOLDS)
    parts = _mechanic_parts(jokers, (
    ...
    if score:
        parts.append(state_contribution(
            "deck:enhanced_card_density", "Enhanced-card density", score,
            mechanic="enhanced_card_density",
        ))
    return finalize_development("enhanced_cards", parts, ENHANCED_CARDS_THRESHOLDS)
