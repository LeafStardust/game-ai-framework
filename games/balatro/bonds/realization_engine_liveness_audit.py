...
def realize_cash_live(d,s):
 j=_jokers(s);money=int(getattr(s,"money",0) or 0);hand=[c for c in _cards(s,"hand","current_hand","cards_in_hand") if not _debuffed(c)];deck=_cards(s,"owned_deck","deck");par=_has(j,"pareidolia");faces=bool(hand) if par else any(not _stone(c) and str(getattr(c,"rank","") or "").upper() in {"J","Q","K"} for c in hand);nines=any(not _stone(c) and str(getattr(c,"rank","") or "").upper()=="9" for c in deck);hist=getattr(s,"unique_planets_used",getattr(s,"satellite_planets_used",None));sat_ok=hist is not None and int(hist or 0)>0
 if not _has_window(s):
  deck_known=any(hasattr(s,n) for n in ("owned_deck","deck"));src=sum((_has(j,"goldenjoker","rocket"),_has(j,"tothemoon") and money>=5,_has(j,"cloud9") and (nines or not deck_known),_has(j,"satellite") and sat_ok,_has(j,"reservedparking") and faces));active=(_has(j,"bull","bootstraps") and money>=25) or src>0;return _finish(d,active,src>=2 or (_has(j,"bull","bootstraps") and money>=75))
