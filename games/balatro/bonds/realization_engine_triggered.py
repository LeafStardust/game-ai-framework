...
def _cards(state,*names):
 for name in names:
  if hasattr(state,name):return list(getattr(state,name,None) or ())
 return []
...
def realize_discard_triggered(dev,state):
 ...
 live=[c for c in discarded if not _debuffed(c)];identity=[c for c in live if not _stone(c)];yorick=_has(jokers,"yorick");castle_suit=str(getattr(state,"castle_suit",getattr(state,"current_castle_suit",getattr(state,"castle_target_suit",""))) or "").lower();castle=_has(jokers,"castle") and bool(castle_suit) and any(_suit(c)==castle_suit or _enh(c)=="wild" for c in live);rebate_rank=str(getattr(state,"mail_in_rebate_rank",getattr(state,"rebate_rank","")) or "").upper();rebate=_has(jokers,"mailinrebate") and bool(rebate_rank) and any(_rank(c)==rebate_rank for c in identity);par=_has(jokers,"pareidolia");face_count=len(live) if par else sum(_rank(c) in {"J","Q","K"} for c in identity);faceless=_has(jokers,"facelessjoker") and face_count>=3;hitroad=_has(jokers,"hittheroad") and any(_rank(c)=="J" for c in identity);sources=sum((yorick,castle,rebate,faceless,hitroad));return _finish(dev,sources>0,sources>=2)
