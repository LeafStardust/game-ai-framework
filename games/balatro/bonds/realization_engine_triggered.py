...
def realize_discard_triggered(dev,state):
    ...
    nonstone=[c for c in discarded if not _stone(c)];yorick=_has(jokers,"yorick") and bool(discarded);castle_suit=str(getattr(state,"castle_suit",getattr(state,"current_castle_suit","")) or "").lower();castle=_has(jokers,"castle") and bool(castle_suit) and any((_suit(c)==castle_suit or _enh(c)=="wild") for c in nonstone);rebate_rank=str(getattr(state,"mail_in_rebate_rank",getattr(state,"rebate_rank","")) or "").upper();rebate=_has(jokers,"mailinrebate") and bool(rebate_rank) and any(_rank(c)==rebate_rank for c in nonstone);par=_has(jokers,"pareidolia");face_count=len(discarded) if par else sum(1 for c in nonstone if _rank(c) in {"J","Q","K"});faceless=_has(jokers,"facelessjoker") and face_count>=3;hitroad=_has(jokers,"hittheroad") and any(_rank(c)=="J" for c in nonstone);sources=sum((yorick,castle,rebate,faceless,hitroad));return _finish(dev,sources>0,sources>=2)
