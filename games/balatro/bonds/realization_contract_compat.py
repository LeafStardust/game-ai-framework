...
def realize_deck_thinning(d,s):
 ...
 first_discard=bool(getattr(s,"first_discard_available",int(getattr(s,"discards_used_this_round",0) or 0)==0));first_hand=bool(getattr(s,"first_hand_available",int(getattr(s,"hands_played_this_round",0) or 0)==0));explicit_discard=any(hasattr(s,n) for n in ("cards_to_discard","selected_cards"));explicit_play=any(hasattr(s,n) for n in ("cards_to_play","scoring_cards","played_cards","current_played_cards"));sd=_cards(s,"cards_to_discard","selected_cards");sp=_cards(s,"cards_to_play","selected_cards");trading=_has(j,"tradingcard") and first_discard and ((len(sd)==1) if explicit_discard else True);six=_has(j,"sixthsense") and first_hand and ((len(sp)==1 and not _debuffed(sp[0]) and not _stone(sp[0]) and str(getattr(sp[0],"rank","") or "")=="6") if explicit_play and sp else True);active=erosion or trading or six or (reduction>0 and d.rank>=BondRank.R2);return _finish(d,active,reduction>=12 and (erosion or trading or six))
...
def realize_discard(d,s):
 j=list(getattr(s,"jokers",()) or ());completed=any(hasattr(s,n) for n in ("discarded_cards","current_discard_cards"));prospective=hasattr(s,"cards_to_discard");cards=_cards(s,"discarded_cards","current_discard_cards","cards_to_discard")
 # A completed discard event remains mechanically realized even if the discard
 # counter has already decremented to zero. Prospective lines still require a
 # remaining discard resource.
 if not completed:
  left=int(getattr(s,"discards_left",getattr(s,"discards_remaining",0)) or 0)
  if left<=0:return _finish(d,False)
 if not completed and not prospective:return _finish(d,_has(j,"yorick","castle","mailinrebate","facelessjoker","hittheroad"))
 if not cards:return _finish(d,False)
 live=[c for c in cards if not _debuffed(c)];identity=[c for c in live if not _stone(c)];par=_has(j,"pareidolia");faces=len(live) if par else sum(str(getattr(c,"rank","") or "").upper() in {"J","Q","K"} for c in identity);jacks=sum(str(getattr(c,"rank","") or "").upper()=="J" for c in identity);rank=str(getattr(s,"mail_in_rebate_rank",getattr(s,"rebate_rank","")) or "").upper();reb=bool(rank) and any(str(getattr(c,"rank","") or "").upper()==rank for c in identity);suit=str(getattr(s,"castle_suit",getattr(s,"current_castle_suit",getattr(s,"castle_target_suit",""))) or "").lower();castle=bool(suit) and any(str(getattr(c,"suit","") or "").lower()==suit or str(getattr(c,"enhancement","") or "").lower()=="wild" for c in live);src=sum((_has(j,"yorick"),_has(j,"facelessjoker") and faces>=3,_has(j,"hittheroad") and jacks>0,_has(j,"mailinrebate") and reb,_has(j,"castle") and castle));return _finish(d,src>0,src>=2)
