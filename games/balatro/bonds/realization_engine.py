...
def realize_discard(dev,state):
 j=_jokers(state);d=_cards(state,"discarded_cards","current_discard_cards","cards_to_discard");left=int(getattr(state,"discards_left",getattr(state,"discards_remaining",0)) or 0)
 if not d or left<=0:return _finish(dev,False)
 par=_has(j,"pareidolia");faces=len(d) if par else sum(1 for c in d if not _stone(c) and str(getattr(c,"rank","") or "").upper() in {"J","Q","K"});jacks=sum(1 for c in d if not _stone(c) and str(getattr(c,"rank","") or "").upper()=="J");rank=str(getattr(state,"mail_in_rebate_rank",getattr(state,"rebate_rank","")) or "").upper();reb=bool(rank) and any(not _stone(c) and str(getattr(c,"rank","") or "").upper()==rank for c in d);suit=str(getattr(state,"castle_suit",getattr(state,"castle_target_suit","")) or "").lower();castle=bool(suit) and any(not _stone(c) and (str(getattr(c,"suit","") or "").lower()==suit or str(getattr(c,"enhancement","") or "").lower()=="wild") for c in d);src=sum((_has(j,"yorick"),_has(j,"facelessjoker") and faces>=3,_has(j,"hittheroad") and jacks>0,_has(j,"mailinrebate") and reb,_has(j,"castle") and castle));return _finish(dev,src>0,src>=2)
...
def realize_card_destruction(dev,state):
 j=_jokers(state);hand=_cards(state,"hand","current_hand","cards_in_hand");fd=bool(getattr(state,"first_discard_available",int(getattr(state,"discards_used_this_round",0) or 0)==0));fh=bool(getattr(state,"first_hand_available",int(getattr(state,"hands_played_this_round",0) or 0)==0));sd=_cards(state,"selected_cards","cards_to_discard");sp=_cards(state,"cards_to_play","selected_cards");tr=_has(j,"tradingcard") and fd and len(sd)==1;six=_has(j,"sixthsense") and fh and len(sp)==1 and not _stone(sp[0]) and str(getattr(sp[0],"rank",""))=="6";raw=getattr(state,"scoring_cards",None);pool=list(raw or ()) if raw is not None else (_cards(state,"played_cards","current_played_cards") or hand);glass=_has(j,"glassjoker") and any(str(getattr(c,"enhancement","") or "").lower()=="glass" for c in pool);canio=_has(j,"canio") and int(getattr(state,"cards_destroyed",0) or 0)>0;a=tr or six or glass or canio;return _finish(dev,a,sum((tr,six,glass,canio))>=2)
def realize_hand_repetition(dev,state):
 j=_jokers(state);cur=str(getattr(state,"current_hand_type",getattr(state,"last_hand_type","")) or "").upper().replace(" ","_");counts=getattr(state,"hand_play_counts",{}) or {};norm={str(k).upper().replace(" ","_"):int(v or 0) for k,v in counts.items()};prev=str(getattr(state,"previous_hand_type","") or "").upper().replace(" ","_");played_now=bool(cur) and bool(_cards(state,"scoring_cards","played_cards","current_played_cards"));prior=max(0,norm.get(cur,0)-1) if played_now and norm else norm.get(cur,0) if cur else 0;repeated=bool(cur) and (prior>0 if norm else cur==prev);cardsharp=_has(j,"cardsharp") and played_now and repeated;supernova=_has(j,"supernova") and played_now and norm.get(cur,0)>0;a=cardsharp or supernova;return _finish(dev,a,a and norm.get(cur,0)>=18)
