...
def realize_cash(dev,state):
 money=int(getattr(state,"money",0) or 0);j=_jokers(state);hand=_cards(state,"hand","current_hand","cards_in_hand");deck=_cards(state,"owned_deck","deck");par=_has(j,"pareidolia");pay=_has(j,"bull","bootstraps");un=_has(j,"rocket","goldenjoker");interest=_has(j,"tothemoon") and money>=5;faces=bool(hand) if par else any(not _debuffed(c) and not _stone(c) and str(getattr(c,"rank","") or "").upper() in {"J","Q","K"} for c in hand);parking=_has(j,"reservedparking") and faces;cloud=_has(j,"cloud9") and any(not _stone(c) and str(getattr(c,"rank","") or "").upper()=="9" for c in deck);hist=getattr(state,"unique_planets_used",getattr(state,"satellite_planets_used",None));sat=_has(j,"satellite") and (hist is None or int(hist or 0)>0);src=sum((un,interest,parking,cloud,sat));return _finish(dev,(pay and money>=25) or src>0,(pay and money>=75) or src>=2 or (src>0 and money>=50))
...
def realize_vampire(dev,state):
 ...
 hand=_cards(state,"hand","current_hand","cards_in_hand");deck=_cards(state,"owned_deck","deck");sc=_cards(state,"scoring_cards","played_cards","current_played_cards");feed_pool=sc or hand or deck;feed=sum(1 for c in feed_pool if not _debuffed(c) and str(getattr(c,"enhancement","") or "").strip());mi=next((i for i,x in enumerate(j) if "midasmask" in _name(x)),None);before=mi is not None and mi<vi;par=_has(j,"pareidolia");pool=sc or hand or deck;face=any(not _debuffed(c) and (par or (not _stone(c) and str(getattr(c,"rank","") or "").upper() in {"J","Q","K"})) for c in pool);renew=before and face;a=feed>0 or renew;return _finish(dev,a,feed>=2 or (renew and int(getattr(state,"vampire_enhancements_consumed",0) or 0)>=15))
