...
def realize_cash(dev,state):
 money=int(getattr(state,"money",0) or 0);j=_jokers(state);hand=_cards(state,"hand","current_hand","cards_in_hand");deck_known=hasattr(state,"owned_deck") or hasattr(state,"deck");deck=_cards(state,"owned_deck","deck");par=_has(j,"pareidolia");pay=_has(j,"bull","bootstraps");un=_has(j,"rocket","goldenjoker");interest=_has(j,"tothemoon") and money>=5;faces=any(not _debuffed(c) for c in hand) if par else any(not _debuffed(c) and not _stone(c) and str(getattr(c,"rank","") or "").upper() in {"J","Q","K"} for c in hand);parking=_has(j,"reservedparking") and faces;cloud=_has(j,"cloud9") and ((not deck_known) or any(not _stone(c) and str(getattr(c,"rank","") or "").upper()=="9" for c in deck));hist=getattr(state,"unique_planets_used",getattr(state,"satellite_planets_used",None));sat=_has(j,"satellite") and (hist is None or int(hist or 0)>0);src=sum((un,interest,parking,cloud,sat));return _finish(dev,(pay and money>=25) or src>0,(pay and money>=75) or src>=2 or (src>0 and money>=50))
...
def realize_joker_sacrifice(dev,state):
 j=_jokers(state);i=next((i for i,x in enumerate(j) if "ceremonialdagger" in _name(x)),None);target_joker=j[i+1] if i is not None and i+1<len(j) else None;target=target_joker is not None and not bool(getattr(target_joker,"eternal",False) or getattr(target_joker,"is_eternal",False));fodder=target and (bool(getattr(state,"sacrificable_joker_available",False)) or "riffraff" in _name(target_joker) or not bool(getattr(target_joker,"eternal",False) or getattr(target_joker,"is_eternal",False)));kind=_name(getattr(state,"blind_type",getattr(state,"blind_kind",getattr(state,"selected_blind_type",""))));boss=bool(getattr(state,"is_boss_blind",False)) or "boss" in kind;mad=_has(j,"madness");pending=bool(getattr(state,"blind_selection_pending",True));a=fodder or (mad and pending and not boss);return _finish(dev,a,a and int(getattr(state,"jokers_destroyed",0) or 0)>=6)
