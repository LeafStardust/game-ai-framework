from types import SimpleNamespace

from games.balatro.bond_prescription_policy import prescription_bonus
from games.balatro.bond_shop_health_policy import (
    StrategyHealthProvenance,
    clear_strategy_health,
    last_strategy_health,
)
from games.balatro.bonds.evaluation import evaluate_bond_composition
from games.balatro.bonds.motifs import MotifState
from games.balatro.live.strategy_health import StrategyHealthMode
from games.balatro.state import BalatroState


def _card(rank, enhancement=None, seal=None):
    return SimpleNamespace(rank=rank, enhancement=enhancement, seal=seal, suit="Spades")


def _joker(name):
    return type(name.replace(" ", ""), (), {})()


def _state():
    state = BalatroState()
    state.phase = "SHOP"
    state.deck_name = "RED"
    state.stake_name = "WHITE"
    state.ante = 3
    state.round = 5
    state.owned_deck = list(state.deck)
    return state


def test_shop_health_cache_rejects_unrelated_round_or_run_identity(monkeypatch):
    import games.balatro.bond_shop_health_policy as policy

    matching = _state()
    health = SimpleNamespace(mode=StrategyHealthMode.REPAIR)
    monkeypatch.setattr(policy, "_LAST_STRATEGY_HEALTH", health)
    monkeypatch.setattr(
        policy,
        "_LAST_STRATEGY_HEALTH_PROVENANCE",
        StrategyHealthProvenance("RED", "WHITE", 3, 5),
    )
    assert last_strategy_health(matching) is health

    different_round = _state()
    different_round.round = 6
    assert last_strategy_health(different_round) is None

    different_deck = _state()
    different_deck.deck_name = "BLUE"
    assert last_strategy_health(different_deck) is None

    different_stake = _state()
    different_stake.stake_name = "BLACK"
    assert last_strategy_health(different_stake) is None


def test_clear_strategy_health_removes_cached_authority(monkeypatch):
    import games.balatro.bond_shop_health_policy as policy

    monkeypatch.setattr(policy, "_LAST_STRATEGY_HEALTH", SimpleNamespace(mode=StrategyHealthMode.SURVIVE))
    monkeypatch.setattr(
        policy,
        "_LAST_STRATEGY_HEALTH_PROVENANCE",
        StrategyHealthProvenance("RED", "WHITE", 2, 4),
    )
    clear_strategy_health()
    assert policy._LAST_STRATEGY_HEALTH is None
    assert policy._LAST_STRATEGY_HEALTH_PROVENANCE is None


def test_baron_mime_steel_scenario_activates_and_prescribes_engine_cards():
    state = _state()
    state.jokers = [_joker("Baron"), _joker("Mime")]
    state.owned_deck = [
        *[_card("K") for _ in range(4)],
        _card("2", enhancement="Steel"),
        _card("3", enhancement="Steel"),
    ]
    _, composition = evaluate_bond_composition(state)
    motif = next(m for m in composition.motifs if m.motif_id == "baron_mime_steel")
    assert motif.state >= MotifState.ACTIVE

    steel_bonus, _ = prescription_bonus(state, kind="TAROT", label="The Chariot")
    king_bonus, _ = prescription_bonus(
        state,
        kind="PLAYING_CARD",
        label="King",
        playing_card=_card("K", enhancement="Steel", seal="Red"),
    )
    assert steel_bonus > 0.0
    assert king_bonus > steel_bonus


def test_burnt_target_planet_uses_evaluated_target(monkeypatch):
    import games.balatro.bond_prescription_policy as policy

    state = _state()
    monkeypatch.setattr(policy, "_active_motif_ids", lambda _state: frozenset({"burnt_target_level"}))
    monkeypatch.setattr(policy, "_burnt_target_hand", lambda _state: "PAIR")

    mercury_bonus, mercury_notes = prescription_bonus(state, kind="PLANET", label="Mercury")
    pluto_bonus, _ = prescription_bonus(state, kind="PLANET", label="Pluto")
    assert mercury_bonus > 0.0
    assert pluto_bonus == 0.0
    assert any("PAIR" in note for note in mercury_notes)


def test_photograph_chad_red_seal_targets_face_cards(monkeypatch):
    import games.balatro.bond_prescription_policy as policy

    state = _state()
    monkeypatch.setattr(
        policy,
        "_active_motif_ids",
        lambda _state: frozenset({"photograph_hanging_chad"}),
    )
    face_bonus, face_notes = prescription_bonus(
        state,
        kind="SPECTRAL",
        label="Deja Vu",
        cards=(_card("Q"),),
    )
    plain_bonus, _ = prescription_bonus(
        state,
        kind="SPECTRAL",
        label="Deja Vu",
        cards=(_card("7"),),
    )
    assert face_bonus > plain_bonus > 0.0
    assert any("face card" in note.lower() for note in face_notes)


def test_vampire_midas_prefers_face_and_enhanced_feedstock(monkeypatch):
    import games.balatro.bond_prescription_policy as policy

    state = _state()
    monkeypatch.setattr(policy, "_active_motif_ids", lambda _state: frozenset({"vampire_midas"}))
    feed_bonus, notes = prescription_bonus(
        state,
        kind="PLAYING_CARD",
        label="Enhanced King",
        playing_card=_card("K", enhancement="Gold"),
    )
    plain_bonus, _ = prescription_bonus(
        state,
        kind="PLAYING_CARD",
        label="Seven",
        playing_card=_card("7"),
    )
    assert feed_bonus > 0.0
    assert plain_bonus == 0.0
    assert any("feed" in note.lower() for note in notes)


def test_hack_red_seal_targets_low_rank_trigger_cards(monkeypatch):
    import games.balatro.bond_prescription_policy as policy

    state = _state()
    monkeypatch.setattr(
        policy,
        "_active_motif_ids",
        lambda _state: frozenset({"low_rank_hack_retrigger"}),
    )
    low_bonus, low_notes = prescription_bonus(
        state,
        kind="SPECTRAL",
        label="Deja Vu",
        cards=(_card("3"),),
    )
    high_bonus, _ = prescription_bonus(
        state,
        kind="SPECTRAL",
        label="Deja Vu",
        cards=(_card("9"),),
    )
    assert low_bonus > high_bonus > 0.0
    assert any("2-5" in note for note in low_notes)


def test_same_red_seal_tool_has_context_specific_rationale(monkeypatch):
    import games.balatro.bond_prescription_policy as policy

    state = _state()
    monkeypatch.setattr(policy, "_active_motif_ids", lambda _state: frozenset({"baron_mime_steel"}))
    _, baron_notes = prescription_bonus(
        state,
        kind="SPECTRAL",
        label="Deja Vu",
        cards=(_card("K"),),
    )
    monkeypatch.setattr(policy, "_active_motif_ids", lambda _state: frozenset({"low_rank_hack_retrigger"}))
    _, hack_notes = prescription_bonus(
        state,
        kind="SPECTRAL",
        label="Deja Vu",
        cards=(_card("3"),),
    )
    assert any("Baron-Mime-Steel" in note for note in baron_notes)
    assert any("Hack" in note for note in hack_notes)
