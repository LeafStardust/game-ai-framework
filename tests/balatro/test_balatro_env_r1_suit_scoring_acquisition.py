...
@pytest.mark.parametrize("joker_type", SUIT_SCORING_TYPES)
def test_balatro_env_r1_suit_scoring_purchase_is_exact_inventory_only(joker_type):
    state = _shop_state()
    state.shop_jokers = [_priced(joker_type)]
    run = HeadlessRunState(public=state, seed=31)
...
@pytest.mark.parametrize("joker_type", SUIT_SCORING_TYPES)
def test_balatro_env_r1_suit_scoring_editions_remain_fail_closed(joker_type):
    state = _shop_state()
    joker = _priced(joker_type)
    joker.edition = "Negative"
    state.shop_jokers = [joker]
    run = HeadlessRunState(public=state, seed=32)
    engine = ShopTransitionEngine()
    action = EnvAction.from_alias("BUY_JOKER", {"slot": 0})
