...
class _Evaluator:
    def evaluate(self, calibration):
        assert calibration.synergy_bonus == 1.5
        metrics = BatchMetrics.from_episodes(
...
def test_live_objective_records_unseeded_session_and_run_provenance(tmp_path: Path):
    ...
    value = make_live_phase_a_objective(config, _Evaluator())(trial)
