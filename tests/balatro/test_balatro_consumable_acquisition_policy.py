...
def test_d4_hermit_buy_and_use_uses_post_purchase_d5_timing():
    ...
    assert any(
        "delegated to post-purchase held-consumable timing" in note
        for note in decision.selected.rationale
    )
    assert any("maximum-value $10 threshold" in note for note in decision.selected.rationale)
...
def test_d4_full_slots_can_buy_and_use_positive_hermit_below_peak():
    ...
    assert any("full consumable slots" in note for note in decision.selected.rationale)


def test_d4_full_slots_obey_d5_immediate_gain_threshold_instead_of_forcing_use():
    state = _state(money=9, slots=1)
    state.consumables = [HighPriestess()]
    candidate = Hermit()
    candidate.price = 3
    timing = LiveConsumableTimingPolicy(
        use_thresholds=ConsumableUseThresholds(
            minimum_immediate_gain=7.0,
        )
    )
...
def test_d4_free_slot_d5_block_suppresses_buy_and_use_but_preserves_buy_option():
    state = _state(money=13)
    candidate = Hermit()
    candidate.price = 3
    timing = LiveConsumableTimingPolicy(
        use_thresholds=ConsumableUseThresholds(
            minimum_immediate_gain=11.0,
        )
    )
