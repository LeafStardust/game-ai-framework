from types import SimpleNamespace

import games.balatro.build_health_policy as policy
from games.balatro.build_health import BuildHealth


def _health():
    return BuildHealth(
        total=60.0,
        survival=80.0,
        immediate=80.0,
        scaling=60.0,
        coherence=50.0,
        runway=50.0,
        critical=False,
        scaling_deficit=False,
        warnings=(),
        engines=(),
    )


class _CountingHealth:
    def __init__(self):
        self.calls = 0

    def evaluate(self, state, *, strategy_tracker=None):
        del state, strategy_tracker
        self.calls += 1
        return _health()


def _state():
    joker = SimpleNamespace(
        name="Stateful",
        counter=1,
        mult=0,
        chips=0,
        x_mult=1.0,
        eternal=False,
        edition=None,
    )
    return SimpleNamespace(
        ante=4,
        round=2,
        phase="SHOP",
        money=30,
        score=0,
        blind_score=5000,
        hands_remaining=0,
        discards_remaining=0,
        hand_size=8,
        joker_slots=5,
        consumable_slots=2,
        last_played_hand=None,
        boss_name=None,
        boss_blind_state_observed=False,
        boss_blind_hands=set(),
        boss_blind_only_hand=None,
        jokers=[joker],
        consumables=[SimpleNamespace(name="The Hermit", used=False)],
        vouchers=[SimpleNamespace(name="Paint Brush")],
        hand_levels={"PAIR": 1},
        hand_play_counts={"PAIR": 0},
        round_hand_play_counts={"PAIR": 0},
        owned_deck=[],
        deck=[],
    )


def test_cache_invalidates_when_opening_hand_size_changes(monkeypatch):
    health = _CountingHealth()
    monkeypatch.setattr(policy, "_HEALTH", health)
    owner = SimpleNamespace()
    state = _state()

    policy._cached_health(owner, state, None)
    policy._cached_health(owner, state, None)
    state.hand_size = 9
    policy._cached_health(owner, state, None)

    assert health.calls == 2


def test_cache_invalidates_on_public_stateful_joker_field_change(monkeypatch):
    health = _CountingHealth()
    monkeypatch.setattr(policy, "_HEALTH", health)
    owner = SimpleNamespace()
    state = _state()

    policy._cached_health(owner, state, None)
    state.jokers[0].counter = 2
    policy._cached_health(owner, state, None)

    assert health.calls == 2


def test_cache_invalidates_on_round_history_or_held_resource_change(monkeypatch):
    health = _CountingHealth()
    monkeypatch.setattr(policy, "_HEALTH", health)
    owner = SimpleNamespace()
    state = _state()

    policy._cached_health(owner, state, None)
    state.round_hand_play_counts["PAIR"] = 1
    policy._cached_health(owner, state, None)
    state.consumables[0].used = True
    policy._cached_health(owner, state, None)
    state.vouchers.append(SimpleNamespace(name="Palette"))
    policy._cached_health(owner, state, None)

    assert health.calls == 4


def test_cache_invalidates_when_tracker_primary_history_changes(monkeypatch):
    health = _CountingHealth()
    monkeypatch.setattr(policy, "_HEALTH", health)
    owner = SimpleNamespace()
    state = _state()
    tracker = SimpleNamespace(
        _last_dominant_strategy_id="flush",
        _last_relevant_strategy_ids=("pair",),
    )

    policy._cached_health(owner, state, tracker)
    policy._cached_health(owner, state, tracker)
    tracker._last_dominant_strategy_id = "straight"
    policy._cached_health(owner, state, tracker)

    assert health.calls == 2
