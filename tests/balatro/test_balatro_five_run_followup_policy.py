...
from games.balatro.actions import DISCARD_CARDS, PLAY_CARDS, SELECT_PACK_CARD, SKIP_BOOSTER, BalatroAction
from games.balatro.card import BalatroCard
from games.balatro.five_run_followup_policy import _realized_joker_weakness, _roster_pressure, _should_force_roster_reroll
...
class RedCardJoker:
    name = "Red Card"
class BannerJoker: pass
class GoldenJoker: pass
class AbstractJoker: pass
...
class _RecoveryEvaluator:
    ...
    def evaluate(self, state, action):
        return 100.0 if action.name == DISCARD_CARDS else 80.0


def _plan(action, expected_score, *, expected_progress=0.0, clear_probability=0.0):
    return LiveBlindPlan(
        action=action,
        value=LiveBlindPlanValue(
            clear_probability=float(clear_probability), expected_progress=float(expected_progress),
            expected_score=float(expected_score), expected_hands_remaining=3.0, expected_discards_remaining=3.0,
        ), horizon=1, exact=True, candidate_count=2,
    )


def test_red_card_forces_skip_over_more_valuable_pack_choice() -> None:
    state = BalatroState(); state.phase = "TAROT_PACK"; state.jokers = [RedCardJoker()]
    skip = BalatroAction(SKIP_BOOSTER); take = BalatroAction(SELECT_PACK_CARD, target=object())
...
def test_pace_play_avoids_all_debuffed_hand_when_active_alternative_meets_pace() -> None:
    state = BalatroState(); state.score = 0; state.hands_remaining = 4; state.discards_remaining = 3
    ...
    bad_action = BalatroAction(PLAY_CARDS, cards=debuffed); good_action = BalatroAction(PLAY_CARDS, cards=active)
    decision = LiveHandActionPolicy(evaluator=_Evaluator()).decide(
        state, [_plan(bad_action, 200, expected_progress=1.0), _plan(good_action, 150)]
    )
...
def test_cash_rich_full_weak_roster_forces_one_upgrade_search_window() -> None:
    state = BalatroState(); state.phase = "SHOP"; state.ante = 5; state.round_num = 11; state.money = 30; state.joker_slots = 5
    state.jokers = [BannerJoker(), GoldenJoker(), SimpleNamespace(name="Red Card", public_state={"mult": 0}), AbstractJoker(), AbstractJoker()]
...
def test_final_discard_is_preserved_when_recovery_gain_is_small() -> None:
    state = BalatroState(); state.score = 0; state.hands_remaining = 4; state.discards_remaining = 1
    state.blind = SimpleNamespace(requirement=400)
    play_action = BalatroAction(PLAY_CARDS, cards=[BalatroCard("A", "Hearts")])
    discard_action = BalatroAction(DISCARD_CARDS, cards=[BalatroCard("2", "Clubs")])
    decision = LiveHandActionPolicy(evaluator=_RecoveryEvaluator()).decide(state, [_plan(play_action, 80), _plan(discard_action, 90)])
