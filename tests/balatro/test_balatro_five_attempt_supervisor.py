...
def test_fifth_completed_attempt_suppresses_attempt_six_restart():
    control = _Control()
    restart_calls = []
    ...
    supervisor._attempts[:] = [_Attempt() for _ in range(5)]
...
def test_restart_still_occurs_before_attempt_cap():
    ...
    supervisor._attempts[:] = [_Attempt() for _ in range(4)]
