...
def test_ancestor_evidence_does_not_auto_activate_specific_children():
    ...
    assert scores["core"].effective_score == 5.0
...
def test_specific_child_evidence_suppresses_core_fallback_leaf():
    scorer = StrategyTreeEvidenceScorer(_split_topology())
...
def test_leaf_ranking_excludes_internal_and_inactive_nodes():
    ...
    assert all(score.is_leaf for score in ranked)
