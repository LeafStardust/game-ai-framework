...
- [ ] Audit pack and consumable decisions for both unjustified skipping and unjustified speculative spending. All unopened D8 booster families are implementation-audited; remaining explicit blockers include Emperor and permanent hand-size opportunity cost for Ouija/Ectoplasm.
...
- [ ] Run `tests/balatro` on the current semantic-repair HEAD and require green before live validation. The latest validated checkpoint predates the current all-family D8 expectations, D2 played-card-chip/stochastic expectation, Hanged Man/Blue Joker, and subsequent pack corrections.
